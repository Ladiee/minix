...
# define TTY_MAJ 4 /* major device of
console */

int kmemfd , memfd ; /* file descriptors
of [k]mem */

struct pstat { /* structure filled by pstat
() */
dev_t ps_dev ; /* major / minor of
controlling tty */
...
int ps_blocked_on ; /* what is the process
blocked on */
...
vir_bytes ps_ssize ; /* stack size (in
bytes ) */
phys_bytes ps_vtext ; /* virtual text offset
*/
phys_bytes ps_vdata ; /* virtual data offset
*/
phys_bytes ps_vstack ; /* virtual stack
offset */
phys_bytes ps_text ; /* physical text offset */
phys_bytes ps_data ; /* physical data offset */
phys_bytes ps_stack ; /* physical stack
offset */
int ps_recv ; /* process number to
receive from */
time_t ps_utime ; /* accumulated user
time */
time_t ps_stime ; /* accumulated system time */
char * ps_args ; /* concatenated
argument string */
vir_bytes ps_procargs ; /* initial stack frame
from MM *
};
/* ****************************************** */
...
/* Open memory devices and get PS info from the
kernel */
...
/* CICLO QUE IMPRIME OS DADOS PARA O EP */
printf ( " \ n PID CPU_Time Sys_Time Sons_Time P_Stack P_Data P_BSS P_Text Name " ) ;
...
printf ( " %4d %s %s %s 0 x %lx 0 x %lx 0 x %lx 0 x %lx %s \n " ,
buf.ps_pid ,
buff1 ,
buff2 ,
buff3 ,
...
/* Pstat collects info on process number p_nr and returns itin buf.
It is assumed that tasks do not have entries in fproc
mproc
*/
...
fprintf ( stderr , " pstat : %d out of range \ n " , p_nr ) ;
return -1;
}
...
/* Assume no parent when the parent and the child
share the same pid.
